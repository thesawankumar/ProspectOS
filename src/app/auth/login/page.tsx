...
export default function LoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      setSession({ email, name: email.split("@")[0].toUpperCase() });
      toast({
        title: "Login Successful",
        description: `Welcome back to your dashboard workspace.`,
        variant: "success",
      });
  ...
  const handleOAuth = (provider: string) => {
    toast({
      ...
      description: "Secure redirect to OAuth provider.",
      variant: "info",
    });
    setTimeout(() => {
      setSession({ email: `${provider.toLowerCase()}user@prospectos.com`, name: `${provider} Member` });
      toast({
    ...
    <div className="relative min-h-screen flex items-center justify-center bg-gray-50/50 px-6 py-12 overflow-hidden">
      ...
      >
        ...
        <div className="text-center mb-8 space-y-2">
          ...
          <h2 className="text-xl font-bold text-gray-800 tracking-tight pt-2">Welcome Back</h2>
          <p className="text-xs text-gray-400">Log in to manage your outreach campaigns</p>
        </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                ...
                <a href="/auth/forgot-password" className="text-2xs font-semibold text-[#D84B68] hover:text-[#EC9EB2]">
                  Forgot?
                </a>
              </div>
              <Input
        ...
        <p className="text-center text-xs text-gray-500 mt-6 font-medium">
          ...
          <a href="/auth/signup" className="text-[#D84B68] font-bold hover:text-[#EC9EB2]">
            Create Account
          </a>
        </p>
      </motion.div>
